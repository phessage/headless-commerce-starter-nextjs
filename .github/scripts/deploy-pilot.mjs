...
if (process.argv[2] === 'stop') {
  ...
  const items = [
    ...
    { apiVersion: 'networking.k8s.io/v1', kind: 'Ingress', metadata, spec: { ingressClassName: 'nginx-prod', rules: [{ host: 'saas-pilot.1ecomm.com', http: { paths: [{ path: '/', pathType: 'Prefix', backend: { service: { name: 'storefront', port: { number: 80 } } } }] } }] } },
  ];
